...
$tools->showAsServiceArray($tools->processFound($tools->find($tools->buildRegex('{folder}validate/rules/{file}\.php',true),$tools->packages(true)),['validation_{file}','\{NAMESPACE}\{file}']),true);